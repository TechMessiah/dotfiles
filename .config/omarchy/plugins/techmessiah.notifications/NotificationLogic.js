...
// Toasts stack from the top-right corner, under the launcher pill, so the two
// share a spot and a shape. edgeInset is the tuned distance from a screen edge;
// the caller measures it because only QML can read Style. The room the launcher
// pill needs is added to the top margin by the caller, not here, so it never
// leaks into the right-hand inset.
function popupPlacement(barPosition, barClearance, gapsOut, edgeInset) {
  var position = String(barPosition || "top")
  var clearance = Number(barClearance)
  var gap = Number(gapsOut)
  var inset = Number(edgeInset)
  if (!isFinite(clearance)) clearance = 0
  if (!isFinite(gap)) gap = 0
  if (!isFinite(inset)) inset = gap

  // Whichever side the bar occupies has to clear it as well as the inset.
  var top = position === "top" ? Math.max(clearance, inset) : inset
  var right = position === "right" ? Math.max(clearance, inset) : inset
  // Matching the two anchored insets puts the stack's top-right corner on the
  // 45-degree diagonal from the monitor's corner.
  var edge = Math.max(top, right)
