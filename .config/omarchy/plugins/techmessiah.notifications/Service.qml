...
Item {
  ...
  // Sonner-style collapsed card stack: only the newest few toasts render,
  // each layer behind the front one nudged up by a fixed peek offset and
  // scaled down from its bottom edge — not spaced out in a full-height list.
  readonly property int maxVisibleStack: 3
  readonly property int stackPeek: Style.space(10)
  readonly property real stackScaleStep: 0.055
  readonly property real stackOpacityStep: 0.28
  // Shared with the OSD pill (techmessiah.osd/Osd.qml cardWidth) so every card
  // in the bottom-centre column is the same width and overflowing text elides.
  readonly property int cardWidth: Style.space(260)

  // Distance from the screen edges. Mirrors the launcher pill's own margins in
  // techmessiah.osd/Osd.qml — move both together.
  readonly property int edgeInset: Style.space(22)

  // The stack's real distance from the top and right edges — the tuned inset,
  // widened to clear the bar on whichever edge it occupies. Both margins are
  // equal by construction, which is what puts the corner on the diagonal.
  // Exposed so the launcher pill can read it and sit on exactly the same
  // corner instead of re-deriving the same arithmetic.
  readonly property int cornerInset: NotificationLogic.popupPlacement(
    barPosition, barClearance, Style.gapsOut, edgeInset).margins.top
  ...
  // The toast stack's own distance from the bottom edge: the tuned inset,
  // widened to clear the bar only when the bar itself occupies the bottom.
  readonly property int bottomInset: barPosition === "bottom"
    ? Math.max(barClearance, edgeInset) : edgeInset
  ...
  Variants {
    ...
    PanelWindow {
      ...
      // Shadcn Sonner-style collapsed stack: index 0 (newest) sits full size
      // up front; older toasts recede behind it — nudged up a fixed peek,
      // scaled down from their bottom edge, and progressively faded — rather
      // than each getting its own full-height row. z keeps the newest one
      // painted on top regardless of model order.
      Item {
        id: popupStack
        anchors.bottom: parent.bottom
        anchors.horizontalCenter: parent.horizontalCenter
        anchors.bottomMargin: service.bottomInset

        Behavior on anchors.bottomMargin { NumberAnimation { duration: 260; easing.type: Easing.OutCubic } }

        width: service.cardWidth
        // Tall enough for the front card plus the peek strip each layer
        // behind it contributes.
        readonly property real frontHeight: cardRepeater.count > 0 && cardRepeater.itemAt(0)
          ? cardRepeater.itemAt(0).height : 0
        height: frontHeight + Math.max(0, Math.min(popupModel.count, service.maxVisibleStack) - 1) * service.stackPeek
        ...
        Repeater {
          ...
          delegate: Item {
            ...
            // Newest (index 0) always painted on top of older layers.
            z: -cardSlot.index
            // Beyond the visible stack depth, a card still exists — queued,
            // clock paused (see ticking below) — but is fully transparent
            // and click-through.
            enabled: cardSlot.index < service.maxVisibleStack

            readonly property int depth: Math.min(cardSlot.index, service.maxVisibleStack - 1)
            readonly property real restScale: 1 - cardSlot.depth * service.stackScaleStep
            readonly property real restOpacity: cardSlot.index < service.maxVisibleStack
              ? Math.max(0, 1 - cardSlot.index * service.stackOpacityStep)
              : 0

            anchors.bottom: parent.bottom
            anchors.horizontalCenter: parent.horizontalCenter
            anchors.bottomMargin: cardSlot.depth * service.stackPeek
            // Scale from the bottom-center so receding layers shrink upward
            // and inward — the "peeking out from behind" perspective.
            transformOrigin: Item.Bottom
            scale: cardSlot.restScale
            opacity: cardSlot.restOpacity
            ...
            // Only the front card's clock runs — cards behind it are queued,
            // not silently expiring out of sight. When the front toast eases
            // out, the next-newest is promoted and gets its own full look,
            // so the newest-first order and "every toast gets seen" both
            // fall out of the same rule.
            readonly property bool ticking: cardSlot.index === 0
              && cardSlot.lifetime > 0 && !card.hovered && !cardSlot.exiting

            onIndexChanged: if (cardSlot.index === 0) cardSlot.remainingLifetime = 1.0

            // Animate the card out, then drop the row. Holding the row for the
            // duration keeps the slot's height reserved, so the stack collapses
            // only once the card has actually faded.
            function beginExit(reason) {
              if (cardSlot.exiting) return
              cardSlot.exiting = true
              exitAnim.reason = reason
              exitAnim.start()
            }

            SequentialAnimation {
              id: exitAnim
              property string reason: "expire"
              ParallelAnimation {
                NumberAnimation { target: card; property: "opacity"; to: 0; duration: 180; easing.type: Easing.InCubic }
                NumberAnimation { target: card; property: "scale"; to: 0.96; duration: 180; easing.type: Easing.InCubic }
                NumberAnimation { target: card; property: "y"; to: Style.space(12); duration: 180; easing.type: Easing.InCubic }
              }
              ScriptAction {
                script: exitAnim.reason === "dismiss"
            ...
            NotificationCard {
              id: card
              // Starts below its settled spot, transparent and slightly
              // small; enterAnim rises it into place. The slot owns stack
              // position, the card owns its own transform, so entrance
              // never fights the reflow.
              opacity: 0
              scale: 0.94
              y: Style.space(16)
              cardWidth: service.cardWidth
              app: cardSlot.app
              ...
              onCloseRequested: cardSlot.beginExit("dismiss")
              ...
              ParallelAnimation {
                ...
                NumberAnimation { target: card; property: "scale"; to: 1; duration: 220; easing.type: Easing.OutCubic }
                NumberAnimation { target: card; property: "y"; to: 0; duration: 260; easing.type: Easing.OutCubic }
              }
            }
          }
        }
